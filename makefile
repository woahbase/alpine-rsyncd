...
# runtime flags
MOUNTFLAGS := \
	# -v $(CURDIR)/rsyncd:/etc/rsyncd \
	# -v $(CURDIR)/rsyncd.d:/etc/rsyncd.d \
	# -v $(CURDIR)/storage:/storage \
...
OTHERFLAGS := \
	...
	-e PGID=$(PGID) \
	-e PUID=$(PUID) \
	# -e RSYNCD_MAKEDIRS=true \
	# -e RSYNCD_USECHROOT=false \
	# -e RSYNCD_USER=root \
	# -e RSYNCD_GROUP=root \
	# -e RSYNCD_USERPASS=insecurebydefaultroot \
	# -e RSYNC_USER=nobody \
...
# # rsync outputs version to stderr :|
...
$(shell docker run --rm --pull=never \
	...
	--version \
	2>/dev/null \
	| grep -E   '^rsync  version ' \
	| sed -Ee 's_^rsync  version ([0-9\.]+).*_\1_')
endef
# -- }}}
